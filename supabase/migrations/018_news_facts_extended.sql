-- Extiende prode_news_facts para incluir ganadores y puntos de grupos en standings,
-- necesarios para las nuevas historias de Crónica (exactísimo, desmemoriados, signo viviente).
...
league_members as (
  select p.id,
         ...
         coalesce(p.correct_winners_count, 0) as winners,
         coalesce(p.group_qualifier_points, 0) as group_pts
...
select jsonb_build_object(
  ...
  'standings', coalesce((
    select jsonb_agg(jsonb_build_object(
        'team', team_name, 'total', total, 'exacts', exacts,
        'winners', winners, 'groupPts', group_pts)
